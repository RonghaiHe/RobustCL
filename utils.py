...
def draw_RMSE(ax, list_times, list_RMSE, str_color, str_label, bool_label):
    ...
    """
    if bool_label:
        ax.plot(list_times, list_RMSE, '-', linewidth=2,
                c=str_color, label=str_label)
    else:
...
def run(init_X, numbers=10000, flag=0, types=[-1, 0], comm_fail_prob=0, meas_bia_prob=0, Seed=0, mission=0):
    ...
    print(
        f"meas_bia_prob = {meas_bia_prob}, comm_fail_prob = {comm_fail_prob}")
    ...
    if n2:
        for type in types:
            if type == 0:
                algs_robots[0] = [Robot_BDA_EKF(
                    X=init_X[r], _id=r, NUM_ROBOTS=NUM_ROBOTS, flag=flag, LANDMARK_POS=LANDMARK_POS) for r in range(NUM_ROBOTS)]
            elif type == 2:
                algs_robots[2] = [Robot_BDA_EKF_CU(
                    X=init_X[r], _id=r, NUM_ROBOTS=NUM_ROBOTS, flag=flag, LANDMARK_POS=LANDMARK_POS) for r in range(NUM_ROBOTS)]
            elif type == 6:
                algs_robots[6] = [Robot_DMV(X=init_X[r], _id=r, NUM_ROBOTS=NUM_ROBOTS,
                                            flag=flag, LANDMARK_POS=LANDMARK_POS) for r in range(NUM_ROBOTS)]
            elif type == 12:
                algs_robots[12] = [Robot_CI_CU(
                    X=init_X[r], _id=r, NUM_ROBOTS=NUM_ROBOTS, flag=flag, LANDMARK_POS=LANDMARK_POS) for r in range(NUM_ROBOTS)]
            elif type == 28:
                algs_robots[28] = [Robot_GS_LRHKF(
                    initial_s=init_X, _id=r, NUM_ROBOTS=NUM_ROBOTS, flag=flag, LANDMARK_POS=LANDMARK_POS) for r in range(NUM_ROBOTS)]

    dr_robots = None
    if -1 in types:
        dr_robots = [Robot(X=init_X[r], _id=r, NUM_ROBOTS=NUM_ROBOTS)
    ...
    if mission in {4, 5}:
        ...
        if comm_fail_prob == 0.5:
            ...
            ax2.text(0.75, 0.85, r'$\tau={:.{}f}$'.format(
                meas_bia_prob, 2), transform=ax2.transAxes)

        ax2.text(0.75, 0.05, r'$\rho=$' +
                 str(comm_fail_prob), transform=ax2.transAxes)
        ...
        ax.text(0.45, 0.95, r'$\rho=$' +
                str(comm_fail_prob), transform=ax.transAxes)
    ...
    for type in types:
        if type >= 10:
            broadcast_comm_his_GS[type] = np.zeros(
                (NUM_ROBOTS, NUM_ROBOTS), dtype=np.uint16)
    ...
    for count in range(numbers):
        ...
        Rv = 0
        if mission in [1, 2]:
            # v1.1.0 divides 3 parts, change the increase
            Rv = 2.7e-3 * (count - 297)
            if 3*count >= 2*numbers:
                meas_bia_prob = 0
            elif 3*count >= numbers:
                meas_bia_prob = 0.5
        elif mission in [4]:  # in [1, 2, 4]
            Rv = 1.8e-3*count - 0.8
            if 2*count >= numbers:
                meas_bia_prob = 0.5
        elif mission == 5:
...
############## Motion step starts ########################
        ...
        _input = [[np.random.randn()*SIGMA_V_INPUT[r] + E_V[r], np.random.randn()
                   * SIGMA_OMEGA_INPUT[r] + E_OMEGA[r]] for r in range(NUM_ROBOTS)]
        # _input = [[0.2, np.random.randn()*SIGMA_OMEGA_INPUT[r] + E_OMEGA[r]] for r in range(NUM_ROBOTS)]
        for r in range(NUM_ROBOTS):
            if abs(_input[r][0] - E_V[r]) > 3*SIGMA_V_INPUT[r]:
                _input[r][0] = E_V[r]
            if abs(_input[r][1] - E_OMEGA[r]) > 3*SIGMA_OMEGA_INPUT[r]:
                _input[r][1] = E_OMEGA[r]

        for type in algs_robots.keys():
            for robot_i in algs_robots[type]:
                robot_i.reset_rela()
                robot_i.motion(v=_input[robot_i._id][0],
                               omega=_input[robot_i._id][1])
        if -1 in types:
            for robot_i in dr_robots:
                robot_i.motion(v=_input[robot_i._id][0],
                               omega=_input[robot_i._id][1])
...
############ observation step  starts ############
        ...
        # whether the observation is biased
        measure_bias_whether = np.zeros(NUM_ROBOTS*(LANDMARK_NUM), dtype=bool)

        for ind in range(NUM_ROBOTS*(LANDMARK_NUM)):
            if (measure_bias_happen[ind] < meas_bia_prob):
                if (flag == -1):
                    measure_noises[ind, :] = measure_noises[ind,
                                                            :] * BIAS_SIGMA_POSE + MU_POSE * Rv
                elif (flag >= 0):
                    measure_noises[ind, :2] = measure_noises[ind, :2] * \
                        BIAS_SIGMA_RANGE_BEARING + MU_RANGE_BEARING * Rv
                measure_bias_whether[ind] = True
            else:
                if (flag == -1):
                    measure_noises[ind, :] = measure_noises[ind, :] * \
                        np.array([R_1[0, 0], R_1[1, 1], R_1[2, 2]])
                elif (flag >= 0):
                    measure_noises[ind, :2] = measure_noises[ind,
                                                             :2] * np.array([R_0[0, 0], R_0[1, 1]])

        for type in algs_robots.keys():
            for robot_i in algs_robots[type]:
                robot_i.measurement_abso(cla_trues=true_robots,
                                         measure_noises=measure_noises[robot_i._id*(
                                             LANDMARK_NUM):robot_i._id*(LANDMARK_NUM)+(LANDMARK_NUM), :],
        ...
        # whether the observation is biased
        measure_bias_whether = np.zeros(NUM_ROBOTS*(NUM_ROBOTS), dtype=bool)
        ...
        for type in algs_robots.keys():
            ...
            for robot_i in algs_robots[type]:
                robot_i.measurement_rela(cla_trues=true_robots,
                                         measure_noises=measure_noises[robot_i._id*(
                                             NUM_ROBOTS):robot_i._id*(NUM_ROBOTS)+(NUM_ROBOTS), :],
...
############ communication step  starts ############
            ...
            for type in algs_robots.keys():
                ...
                robot_comm_order = sorted(
                    np.arange(NUM_ROBOTS), key=lambda i: communicate_time_copy[:NUM_ROBOTS][i])
                # only for picking which index in communicate_time_copy
                comm_times = np.zeros(NUM_ROBOTS, dtype=np.uint8)
                # Times about send ing the message
                send_times = np.zeros((NUM_ROBOTS, NUM_ROBOTS), dtype=int)
                if type in {0, 1, 3, 4}:  # DCL request, only EKF at one robot

                    # index is asked for belief before filtering by value
                    asked4belief_before = np.ones(NUM_ROBOTS, dtype=int)*-1
                    # received_after = np.zeros(NUM_ROBOTS, dtype=bool) # Received after filtering, and work, so no repeated

                    broadcast_matched = np.zeros(
                        NUM_ROBOTS, dtype=bool)  # For match
                    send_back_index = np.ones(
                        NUM_ROBOTS, dtype=int)*-1  # Index
                    ...
                    while communicate_time_copy[robot_comm_order[0]] < DELTA_T*COMM_RATE:
                        ...
                        if broadcast_matched[robot_i._id]:
                            ...
                            if asked4belief_before[robot_i._id] >= 0:
                                ...
                                if communicate_noises[robot_i._id*NUM_ROBOTS + r, send_times[robot_i._id, r]] > comm_fail_prob:
                                    ...
                                    can = algs_robots[type][r].communicate2_1(
                                        robot_j=robot_i, get_other=False)

                                    if (robot_i._id in algs_robots[type][r].sort_pair_request):
                                        algs_robots[type][r].sort_pair_request.remove(
                                            robot_i._id)
                                    ...
                                    comm_times[r] += 1
                                    communicate_time_copy[r] += communicate_time_copy[NUM_ROBOTS *
                                                                                      comm_times[r] + r]
                                    ...
                                    if robot_i.measuring[r] or can:
                                        ...
                                        communicate_time_copy[robot_comm_order[0]] += communicate_time_copy[NUM_ROBOTS *
                                                                                                            comm_times[robot_comm_order[0]] + robot_comm_order[0]]

                                else:
                                    communicate_time_copy[robot_comm_order[0]] -= (
                                        communicate_time_copy[NUM_ROBOTS*comm_times[robot_comm_order[0]]+robot_comm_order[0]] - DELTA_T*2)
                                    comm_times[robot_comm_order[0]] += 1
                                    communicate_time_copy[robot_comm_order[0]] += communicate_time_copy[NUM_ROBOTS *
                                                                                                        comm_times[robot_comm_order[0]] + robot_comm_order[0]]
                            ...
                            # r is major, robot_i, who? just sender(3rd)
                                ...
                                if communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                    ...
                                    # else:
                                        ...
                                        algs_robots[type][r].communicate2_2(
                                            robot_i=robot_i)
                        ...
                        if pick >= 0 and len(robot_i.sort_pair_request):
                            ...
                            if robot_i.measuring[r] and communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                ...
                                # For match
                                broadcast_matched[r], broadcast_matched[robot_i._id] = True, True
                        ...
                        # Not any other robot needed to EKF
                            ...
                            communicate_time_copy[robot_comm_order[0]] += 2

                        robot_comm_order = sorted(
                            np.arange(NUM_ROBOTS), key=lambda i: communicate_time_copy[:NUM_ROBOTS][i])

                # (DCL no request, origin) now: BDA-CU with request, match
                elif type in {2, 5}:
                    # index is asked for belief before filtering by value
                    asked4belief_before = np.ones(NUM_ROBOTS, dtype=int)*-1
                    broadcast_matched = np.zeros(
                        NUM_ROBOTS, dtype=bool)  # For match
                    broadcast_filtered = np.zeros(
                        (NUM_ROBOTS, NUM_ROBOTS), dtype=bool)  # For match
                                ...
                                if communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                    ...
                                    if can:
                                        ...
                                        if broadcast_filtered[robot_i._id, r]:
                                            algs_robots[type][r].communicate2_2CU(
                                                robot_i=robot_i)
                ...
                # Zhu_Kia from paper, both robot EKF both measurement(If it is, not others needed)
                elif type in {6, 7}:
                    broadcast_matched = np.zeros(
                        NUM_ROBOTS, dtype=bool)  # For match
                    ...
                    while communicate_time_copy[robot_comm_order[0]] < DELTA_T*COMM_RATE:
                        ...
                        if send_back_index[robot_i._id] >= 0:
                            ...
                            if communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                ...
                                can_2 = algs_robots[type][r].communicate1_1(
                                    robot_j=robot_i)
                                if can_2:
                                    algs_robots[type][r].pair_history[robot_i._id] = count
                                algs_robots[type][r].sort_pair_request.remove(
                        ...
                        # for r in robot_i.sort_pair_request:
                            ...
                            if robot_i.measuring[r] and communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                ...
                                if robot_i._id not in algs_robots[type][r].sort_pair_request or \
                                    ...
                                    comm_times[r] += 1
                                    communicate_time_copy[r] = communicate_time_copy[robot_comm_order[0]
                                                                                     ] + communicate_time_copy[NUM_ROBOTS*comm_times[r] + r]
                            ...
                            # Cannot send the message to that robot, still waiting for broadcast_matched
                                ...
                                communicate_time_copy[robot_comm_order[0]] -= (
                                    communicate_time_copy[NUM_ROBOTS*comm_times[robot_comm_order[0]]+robot_comm_order[0]] - DELTA_T*2)
                                comm_times[robot_comm_order[0]] += 1
                                comm_times[robot_comm_order[0]] += communicate_time_copy[NUM_ROBOTS *
                                                                                         comm_times[robot_comm_order[0]] + robot_comm_order[0]]
                ...
                # only communicate once: CI-CU
                    ...
                    for r in range(NUM_ROBOTS):
                        ...
                        for _id in range(NUM_ROBOTS):
                            if _id != r and not algs_robots[type][r].measuring[_id]:
                                broadcast_comm_his_GS[type][r][_id] += numbers

                    while communicate_time_copy[robot_comm_order[0]] < DELTA_T*COMM_RATE:
                        robot_i = algs_robots[type][robot_comm_order[0]]

                        comm_1st_order = sorted(np.arange(
                            NUM_ROBOTS), key=lambda i: broadcast_comm_his_GS[type][robot_i._id][i])
                        if (broadcast_comm_his_GS[type][robot_i._id][comm_1st_order[0]] == count):
                            ...
                            if communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                ...
                                can = algs_robots[type][r].communicate1(
                                    robot_i)
                                # Received and update with others
                                if can:
                                    broadcast_comm_his_GS[type][robot_i._id][r] = count
                                # Received but cannot update, temporally no use
                                else:
                                    broadcast_comm_his_GS[type][robot_i._id][r] += numbers
                            else:
                                communicate_time_copy[robot_comm_order[0]] -= (
                                    communicate_time_copy[NUM_ROBOTS*comm_times[robot_comm_order[0]] + robot_comm_order[0]] - DELTA_T*2)
                        ...
                        robot_comm_order = sorted(
                            np.arange(NUM_ROBOTS), key=lambda i: communicate_time_copy[:NUM_ROBOTS][i])
                    broadcast_comm_his_GS[type] = [
                        [val - numbers if val >= numbers else val for val in row] for row in broadcast_comm_his_GS[type]]
                ...
                # resilient
                    ...
                    while communicate_time_copy[robot_comm_order[0]] < DELTA_T*COMM_RATE:
                        ...
                        if (broadcast_comm_his_GS[type][robot_i._id][comm_1st_order[0]] == count):
                            ...
                            # robot_i -> r, no matter success or fail
                            if communicate_noises[robot_i._id*NUM_ROBOTS+r, send_times[robot_i._id, r]] > comm_fail_prob:
                                can = algs_robots[type][r].communicate1_CI(
                                    robot_i.X_GS_prediction, robot_i.P_GS_prediction, robot_i._id)
                                if can:
                                    broadcast_comm_his_GS[type][robot_i._id][r] = count
                                else:
                                    broadcast_comm_his_GS[type][robot_i._id][r] += numbers
...
############ communication step  ends############
        ...
        if mission in [4, 5]:
            ...
            for type in types:
                ...
                if not type == -1:
                    for robot_i in algs_robots[type]:
                        robot_i.draw(
                            ax2, draw.EACH_COLOR[cou], draw.LABELS[type])
            ...
            ax2.axis('equal')
            if comm_fail_prob == 0.5:
                ax2.text(0.75, 0.9, 'Time: {:.{}f}s'.format(
                    (count + 1)*DELTA_T, 1), transform=ax2.transAxes)
                ax2.text(0.75, 0.85, r'$\tau={:.{}f}$'.format(
                    meas_bia_prob, 2), transform=ax2.transAxes)
            ...
            ax2.text(0.75, 0.05, r'$\rho=$' +
                     str(comm_fail_prob), transform=ax2.transAxes)
            draw.plt.savefig('./video-' + str(mission) + '/Tra-' + str(comm_fail_prob) +
                             '/' + str(count+1).zfill(4) + '.png', dpi=600, bbox_inches='tight')
            draw.plt.close()
                ...
                if not type == -1:
                    for r in range(NUM_ROBOTS):
                        ...
                        if r < 1:
                            bool_label = True
                        draw_RMSE(
                            ax, list_times, list_RMSE[type], draw.EACH_COLOR[cou], draw.LABELS[type], bool_label)
                    cou += 1
            if -1 in types:
                for r in range(NUM_ROBOTS):
                    bool_label = False
                    if r < 1:
                        bool_label = True
                    draw_RMSE(
                        ax, list_times, list_RMSE[-1], draw.EACH_COLOR[cou], draw.LABELS[-1], bool_label)
            ...
            if comm_fail_prob == 0.1:
                ax.set_ylabel('ARMSE [m]')
            ax.set_xlabel('simulation time [s]')
            draw.plt.savefig('./video-' + str(mission) + '/' + str(comm_fail_prob) +
                             '/' + str(count+1).zfill(4) + '.png', dpi=600, bbox_inches='tight')
            draw.plt.close()
    ...
    RMSE_list, ANEES_list, M = storage(
        algs_robots=algs_robots, dr_robots=dr_robots)
...
def experiment0(args):
    '''
    Draw the sample trajectory of all robots in 2D axis
    ...
    comm_fail_prob = parameters.comm_fail_prob
    RMSE_list, ANEES_list, M = run(
        np.array(init_X), numbers, flag, types, comm_fail_prob, meas_bia_prob, 1, 0)
...
def experiment1(args):
    ...
    for _ in range(20):
        ...
        meas_bia_prob = parameters.meas_bia_prob
        RMSE_list, ANEES_list, M = run(init_X=np.array(init_X), numbers=numbers, flag=flag, types=types,
                                       comm_fail_prob=comm_fail_prob, meas_bia_prob=meas_bia_prob, Seed=_+0, mission=1)
        for type in RMSE_list.keys():
            ...
            if not type == -2:
                ...
                RMSE_[type] = np.mean(
                    np.array(RMSE_list[type]), axis=1)  # 10000*9
                R_list[type].append(RMSE_[type])
...
def experiment2(args):
    ...
    for rho in [0.9, 0.5, 0.1]:
        ...
        for _ in range(20):
            ...
            # M: 6*10000*3
            RMSE_list, ANEES_list, M = run(init_X=np.array(init_X), numbers=numbers, flag=flag, types=types,
                                           comm_fail_prob=comm_fail_prob, meas_bia_prob=meas_bia_prob, Seed=_+0, mission=2)
            for type in RMSE_list.keys():
                ...
                M_list[type][rho].append(np.nanmean(
                    np.array(M[type]), axis=0))  # 20*10000*3
...
def experiment3(args):
    ...
    for _ in range(20):
        ...
        for network in range(11):
            ...
            # 9*10000
            RMSE_list, ANEES_list, M = run(init_X=np.array(init_X), numbers=numbers, flag=flag, types=types,
                                           comm_fail_prob=comm_fail_prob, meas_bia_prob=meas_bia_prob, Seed=_, mission=3)
            for type in RMSE_list.keys():
                ...
                if not type == -2:
                    ...
                    total_R_list[type][_].append(
                        np.mean(np.array(RMSE_list[type]), axis=1))  # 20*11*(9,)

                else:
                    RMSE_[type] = np.mean(
                        np.mean(np.array(RMSE_list[type])))  # 10000*9
                    R_list[type][_].append(RMSE_[type])
                    total_R_list[type][_].append(
                        np.mean(np.array(RMSE_list[type]), axis=0))

                if not type == -2:
                    ANEES_[type] = np.mean(np.mean(np.array(ANEES_list[type])))
                    A_list[type][_].append(ANEES_[type])
                    total_A_list[type][_].append(
                        np.mean(np.array(ANEES_list[type]), axis=1))
                else:
                    ANEES_[type] = np.mean(np.mean(np.array(ANEES_list[type])))
                    A_list[type][_].append(ANEES_[type])
                    total_A_list[type][_].append(
                        np.mean(np.array(ANEES_list[type]), axis=0))

    for type in total_R_list.keys():
        aver_R_list[type] = (
            np.mean(np.array(total_R_list[type]), axis=0)).T  # 11*(9,)
        aver_A_list[type] = (np.mean(np.array(total_A_list[type]), axis=0)).T
...
def experiment4(args):
    ...
    RMSE_list, ANEES_list, M = run(
        np.array(init_X), numbers, flag, types, comm_fail_prob, meas_bia_prob, 1, 4)
...
def experiment5(args):
    ...
    RMSE_list, ANEES_list, M = run(
        np.array(init_X), numbers, flag, types, comm_fail_prob, meas_bia_prob, 1, 5)
