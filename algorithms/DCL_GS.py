...
class Robot_GS_EKF:
    def __init__(self, initial_s, _id, NUM_ROBOTS, flag, LANDMARK_POS):
        ...
        self._id = _id
        self.flag = flag
        self.X_GS = np.array(initial_s, dtype = 'float64').reshape((3*self.NUM_ROBOTS, 1))        
        self.P_GS = parameters.P_ALL_INIT.copy()
        
        self.measuring = np.zeros(NUM_ROBOTS, dtype=bool)
        self.LANDMARK_NUM = len(LANDMARK_POS)
        self.LANDMARK_POS = np.array(LANDMARK_POS).reshape((3*self.LANDMARK_NUM, 1))
    ...
    def motion(self, v, omega):
        ...
        for r in range(self.NUM_ROBOTS):
            ...
            if r == self._id:
                ...
                F = np.array([[1,0,-E_V[r] * DELTA_T * s1], [0, 1, E_V[r] * DELTA_T * c1], [0, 0, 1]])
    ...
    def measurement_abso(self, cla_trues, measure_noises, measure_bias_whether=[]):
        ...
        for LM_id in range(self.LANDMARK_NUM):
            _range, bearing, X2_ = parameters.measurement(self.LANDMARK_POS[3*LM_id:3*LM_id+3, 0], That_true)
            if _range <= RB and abs(bearing) <= BB: # self->LM_id
                ...
                if(self.flag>=0):
                    ...
                    self.Z_landmark[2*LM_id, 0] = _range + measure_noises[LM_id,0]
                    self.Z_landmark[2*LM_id+1, 0] = bearing + measure_noises[LM_id,1]
                elif(self.flag==-1):
                    self.Z_landmark[3*LM_id:3*LM_id+3] = (X2_ + measure_noises[LM_id,:]).reshape(-1,1)
    ...
    def measurement_rela(self, cla_trues, measure_noises, measure_bias_whether=[]):
        ...
        # other robots
        for r in range(self.NUM_ROBOTS):
            if(r == self._id): continue
            _range, bearing, X2_ = parameters.measurement(cla_trues[r].X_true, That_true)
    ...
    def abso_meas_correct(self, count=500):
        ...
        '''
        for LM_id in range(self.LANDMARK_NUM):
            if not self.measuring_landmark[LM_id]: continue
            ...
            if(self.flag == 0):
                ...
                dp = self.LANDMARK_POS[3*LM_id:3*LM_id+2] - self.X_GS[3*self._id:3*self._id+2]
                ...
                R2 = (R_ALL_0_LANDMARK[2*LM_id:2*LM_id+2,2*LM_id:2*LM_id+2])**2
            
            elif(self.flag == 1):
                R = R_ALL_0_LANDMARK[2*LM_id:2*LM_id+2,2*LM_id:2*LM_id+2].copy()
                gamma = parameters.rot_mat_2d(self.X_GS[3*self._id+2, 0])[0:2, 0:2]
                J = np.array([[0, -1], [1, 0]])
                dp = self.LANDMARK_POS[3*LM_id:3*LM_id+2] - self.X_GS[3*self._id:3*self._id+2]
                ...
                R[1, 1] = R[1, 1] * rho
                gamma_bearing = parameters.rot_mat_2d(alpha)[0:2,0:2].T # anticlockwise
                ...
                R2 = (R_ALL_1_LANDMARK[3*LM_id:3*LM_id+3,3*LM_id:3*LM_id+3])**2
            
            v = Z_now - Z_cal
            if(self.flag == 0 and abs(v[1, 0])>4):
                if(v[1, 0]>0): 
                    v[1, 0] = v[1, 0] - 2*pi
                else: v[1, 0] = v[1, 0] + 2*pi
    ...
    def rela_meas_correct(self, count, cla_true = []):
        ...
        '''
        for r in range(self.NUM_ROBOTS):
            if not self.measuring[r]: continue
    ...
    def communicate1_CI(self, Xi_GS, Pi_GS, _id):
        ...
        if np.isinf(temp).any() or np.isnan(temp).any(): return
        condition_number = np.linalg.cond(temp)
        if condition_number > 1e4: return 0
    ...
    def calc_RMSE(self, cla_true):
        self.RMSE = (sum((self.X_GS[3*self._id:3*self._id+2,0] - cla_true.X_true[0:2])**2)/2)**(0.5)
    
    def calc_MAEP(self, cla_true):
        self.MAEP = np.linalg.norm(np.squeeze(np.array(self.X_GS[3*self._id:3*self._id+2,0])) - cla_true.X_true[0:2], ord=1)
    ...
    def calc_ANEES(self, cla_true):
        ...
        inv_P = None
        try:
            inv_P = np.linalg.inv(self.P_GS[3*self._id:3*self._id+2, 3*self._id:3*self._id+2])
...
class Robot_GS_LRHKF(Robot_GS_EKF):
    ...
    def abso_meas_correct(self, count=500):
        ...
        for LM_id in range(self.LANDMARK_NUM):
            ...
            if not parameters.is_pos_def(P_pre): return
            ...
            for iter_M_count in range(iter_M):
                if(np.linalg.norm(X_all - X_past) < 1e-5): break
                X_past = X_all.copy()
                ...
                try:
                    ...
                    temp_I = np.linalg.inv(H @ P_pre_ @ Psi_x_I @ P_pre_ @ H.T + R_ @ Psi_y_I @ R_)
                except np.linalg.LinAlgError:
                    if not iter_M_count: total_no = True
                    break
