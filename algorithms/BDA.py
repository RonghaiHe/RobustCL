...
class Robot_BDA_EKF(Robot):
    ...
    def EKF2(self, robot_j, get_other):
        ...
        if(get_other and robot_j.measuring[self._id]): two_meas = True
        ...
        if(self.flag==0): # bearing
            ...
            if two_meas:
                Z_cal = np.concatenate( (Z_cal, [rho, atan2(-delta_y, -delta_x) - self.X_j[2]] ) )  
        ...
        # range-bearing, but transforms into relative position
            ...
            if two_meas:
                ...
                Z_cal = np.concatenate( (Z_cal, rot @ (self.X[:2] - self.X_j[:2]) ) )
                ...
                alpha_cal = atan2(self.X[1] - self.X_j[1], self.X[0] - self.X_j[0]) - self.X_j[2]
