...
class Robot_DMV(Robot):
    ...
    def measurement_abso(self, cla_trues, measure_noises, measure_bias_whether=[]):
        ...
        for LM_id in range(self.LANDMARK_NUM):
            _range, bearing, X2_ = parameters.measurement(self.LANDMARK_POS[3*LM_id:3*LM_id+3, 0], My_true)
            if _range <= RB and abs(bearing) <= BB: # self->LM_id
                ...
                if(self.flag>=0):
                    ...
                    self.Z_landmark[LM_id, 0] = _range + self.measure_noise[LM_id, 0]
                    self.Z_landmark[LM_id, 1] = bearing + self.measure_noise[LM_id, 1]
                elif(self.flag==-1):
    ...
    def measurement_rela(self, cla_trues, measure_noises, measure_bias_whether=[]):
        ...
        for _id in range(self.NUM_ROBOTS):
            if(_id == self._id): continue
            _range, bearing, X2_ = parameters.measurement(cla_trues[_id].X_true, My_true)
        ...
        else: self.sort_pair_request.sort(key = self.sort_by_his) # For Match
    ...
    def abso_meas_correct(self, count=500):
        ...
        for LM_id in range(self.LANDMARK_NUM):
            ...
            if(self.flag == 0):
                ...
                rho, alpha = self.Z_landmark[LM_id, 0], self.Z_landmark[LM_id, 1]
    ...
    def communicate1_1(self, robot_j):
        ...
        else: self.meas_info_kind = 1 # robot_j->I
    ...
    def EKF_DMV(self, robot_j):
        ...
        if(self.flag==0): # range-bearing
            ...
            # delta_x: measurer-measured
                ...
                Z_cal = np.array([rho, atan2(-delta_y, -delta_x) - self.X_j[2]])
            ...
            # Add robot_j's observation
                ...
                Z_cal[:2] = np.array([rho, atan2(delta_y, delta_x) - self.X[2]])
                Z_cal[2:] = np.array([rho, atan2(-delta_y, -delta_x) - self.X_j[2]])
                ...
                v = np.concatenate((self.Z[robot_j._id], robot_j.Z[self._id])) - Z_cal
            ...
            # Add robot_j's observation
                ...
                gamma_bearing1 = parameters.rot_mat_2d(alpha1)[0:2,0:2].T # anticlockwise
                R2[1, 1] = R2[1, 1] * rho1
                R2[:2,:2] = gamma_bearing1 @ R2[:2,:2]**2 @ gamma_bearing1.T

                rho2, alpha2 = robot_j.Z[self._id, 0], robot_j.Z[self._id, 1]
                
                alpha_cal2 = atan2(self.X[1] - self.X_j[1], self.X[0] - self.X_j[0]) - self.X_j[2]
                ...
                Z_now = np.array([rho1 * cos(alpha1), rho1 * sin(alpha1), rho2 * cos(alpha2), rho2 * sin(alpha2)])
                
                gamma_bearing2 = parameters.rot_mat_2d(alpha2)[0:2,0:2].T # anticlockwise
                R2[3, 3] = R2[3, 3] * rho2
        ...
        if(self.flag == 0):
            if(abs(v[1])>4):
                if(v[1]>0): v[1] = v[1] - (2*pi)
                else: v[1] = v[1] + (2*pi)
            if self.meas_info_kind == 2 and abs(v[3])>4:
                if(v[3]>0): v[3] = v[3] - (2*pi)
                else: v[3] = v[3] + (2*pi)
        ...
        if self.bEKF:
            ...
            if(abs(np.linalg.det(S))<1e-7): return 0
            ...
            def fitness(w):
                try:
                    f = np.linalg.inv(w * inv_P_i + (1-w) * H_i.T @ inv_Pj_i @ H_i)
            ...
            temp_I = None
            try:
                temp_I = np.linalg.inv(H_i @ self.P @ H_i.T /w + H_j @ self.P_j @ H_j.T /(1-w) + R2/gamma)
            except np.linalg.LinAlgError:
                return 0
