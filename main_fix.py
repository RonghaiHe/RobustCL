...
MEAS_MODEL = ['range-bearing', 'range-bearing->relative position', 
                      'relative pose']
MISSION_DETAIL = ['ARMSE over time', 'weight of M-estimation over time', 
...
def main(args):
    ...
    print(MISSION_DETAIL[args['mission']-1])
    print('')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--comm_fail_prob', help='set communication failure probability', default=0.9)
    args_ = parser.parse_args()
    args = {
            # Initial poses of 6 robots
            'init_X': [[0,12,0], [1,9,0], [0,6,0], [1,3,0], [0,0,0], [1,-3,0]],
            # Algorithm types, see variable **LABELS**
            # 'types': [28], # For experiment2 only
            'types': [-1, 6, 0, 2, 12, 28], # Final contrast， experiment1&3
